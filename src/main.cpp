...
void windowEventHandler(Event &e, RenderWindow *window) {
...
void drawWindow(Sprite &tileSprite, Sprite &snakeSprite, Sprite &foodSprite,
                RenderWindow *window) {
  ...
  for (int i = 0; i < HOR_SIZE; i++) // sprite drawing function
  {
    ...
    vector<pair<int, int>> *snakePos = snake.getPos();
